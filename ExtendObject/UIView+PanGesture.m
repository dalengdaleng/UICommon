...
@property (nonatomic, weak) id<PanGestureDelegate> panGestureDelegate;
...
- (void)addPanGestureWithEndFrame:(CGRect)endFrame {
    ...
    UIView *panMaskView = [[UIView alloc] initWithFrame:kScreenBounds];//self.superview.bounds
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)sender {
    ...
    if ([self.panGestureDelegate respondsToSelector:@selector(panGesture:didChangeWithView:)]) {
        [self.panGestureDelegate panGesture:self.panGesture didChangeWithView:self];
    }
...
- (void)showGesturedViewWithCompletedBlock:(void(^)(BOOL isFinished))completedBlock {
    if ([self.panGestureDelegate respondsToSelector:@selector(panGesture:willShowTargetView:)]) {
        [self.panGestureDelegate panGesture:self.panGesture willShowTargetView:self];
    }
    ...
    if ([self.panGestureDelegate respondsToSelector:@selector(panGesture:didShowTargetView:)]) {
        [self.panGestureDelegate panGesture:self.panGesture didShowTargetView:self];
    }
    //    }] forKey:@"kPanMaskViewShowAnimation"];
}

- (void)hideGesturedViewWithCompletedBlock:(void(^)(BOOL isFinished))completedBlock {
    if ([self.panGestureDelegate respondsToSelector:@selector(panGesture:willHideTargetView:)]) {
        [self.panGestureDelegate panGesture:self.panGesture willHideTargetView:self];
    }
    self.isAnimating = YES;
    ...
    if ([self.panGestureDelegate respondsToSelector:@selector(panGesture:didHideTargetView:)]) {
        [self.panGestureDelegate panGesture:self.panGestureDelegate didHideTargetView:self];
    }
...
- (void)setPanGestureDelegate:(id<PanGestureDelegate>)panGestureDelegate {
    objc_setAssociatedObject(self, @selector(panGestureDelegate), panGestureDelegate, OBJC_ASSOCIATION_ASSIGN);
}

- (id<PanGestureDelegate>)panGestureDelegate {
